...
ll getIndices(vector<int>& nums, ll guess, ll index) {
    int count=0;
    for (int i = index+1; i < nums.size(); i++) {
        if (abs(guess-nums[i])>abs(guess-nums[index]))count++;
    }
    return count;
}
void solve() {
    ...
    for (int i = 0; i < n; i++) {
            cout << max(getIndices(nums, INT_MAX, i), getIndices(nums, INT_MIN, i)) << " ";
    }
    cout << "\n";
